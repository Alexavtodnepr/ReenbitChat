...
export class ChatFooterComponent implements OnInit, OnChanges {
  ...
  chuckNorris(){
    this.jokeChuck.getJoke().subscribe((res:any)=>{
      const newMessage = {
        date: new Date(),
        text: res.value,
        author: '',
      ...
      setTimeout(() =>{
        ...
        console.log(id)
        this.db.updateChats(id, newMessage)
        this.beep();
        this.setingStorage();
      },10000);
    });
